...
fn render_tabbar(frame: &mut Frame, app: &App, area: Rect) {
    ...
    for (i, buf) in app.editor.buffers.iter().enumerate() {
        let is_active = i == app.editor.active;
        let mod_flag  = if buf.modified { "● " } else { "" };
        let label     = format!(" {}{} ", mod_flag, buf.name);
    ...
    let w = Paragraph::new(line).style(Style::default().bg(BG_PANEL));
...
fn render_editor(frame: &mut Frame, app: &mut App, area: Rect) {
    let cfg          = &app.config.editor;
    let buf          = &app.editor.buffers[app.editor.active];
    ...
    let all_lines_for_hl: Vec<String> = (0..=scroll_top + visible_h)
        .map(|l| buf.get_line(l))
        .collect();

    let ext = app.editor.active_extension();
    let highlighted = app.editor.highlighter.highlight(&all_lines_for_hl, &ext);

    for screen_row in 0..visible_h {
        let abs_line = scroll_top + screen_row;
        let y        = area.top() + screen_row as u16;
        let is_cur   = abs_line == cursor_line;
        let line_bg  = if is_cur && cfg.highlight_line { BG_LINE_HL } else { BG };
        ...
        if cfg.line_numbers && gutter_w > 0 {
            let num_str = if cfg.relative_numbers && !is_cur {
                let rel = (abs_line as isize - cursor_line as isize).unsigned_abs();
        ...
        let text_x0 = area.left() + gutter_w;
        let spans    = highlighted.get(abs_line).map(|v| v.as_slice()).unwrap_or(&[]);
        ...
        'span_loop: for span in spans {
            for ch in span.text.chars() {
                ...
                let sx = text_x0 + col_offset;
                if sx >= area.right() { break 'span_loop; }

                let bg = if ruler > 0 && col_offset == ruler {
                    RULER_COL
                } else {
                    line_bg
                };

                let mut cell_style = span.style.bg(bg);

                let is_cursor_cell = is_cur && col_offset as usize == cursor_col;
                if is_cursor_cell {
    ...
    if app.mode != Mode::CommandPalette {
        let cx = (area.left() + gutter_w + cursor_col as u16).min(area.right().saturating_sub(1));
        let cy = (area.top() + (cursor_line - scroll_top) as u16).min(area.bottom().saturating_sub(1));
...
fn render_file_tree(frame: &mut Frame, app: &App, area: Rect) {
    let focused = app.mode == Mode::FileTree;
...
fn render_diagnostics(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let items: Vec<ListItem> = buf.diagnostics.iter().map(|d| {
        ...
        let label = format!("{} {}:{} {}", icon, d.line + 1, d.col + 1, d.message);
        ListItem::new(label).style(Style::default().fg(col))
    }).collect();

    if items.is_empty() {
        let p = Paragraph::new("No diagnostics")
...
fn render_status_bar(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let mod_flag = if buf.modified { " ●" } else { "" };
    let fname    = format!("  {}{}", buf.name, mod_flag);
    let fn_style = Style::default().fg(if buf.modified { WARN_COL } else { FG }).bg(bar_bg);
    ...
    if let Some(msg) = &app.message {
        let s = format!("   {}", msg);
        let msg_style = Style::default().fg(WARN_COL).bg(bar_bg);
        for ch in s.chars() {
            if x >= area.right() { break; }
            if let Some(cell) = frame.buffer_mut().cell_mut((x, area.top())) {
    ...
    let ext      = app.editor.active_extension();
    let ext_disp = if ext.is_empty() { "text".to_string() } else { ext };
    let can_u    = if buf.can_undo() { "U" } else { "-" };
    let can_r    = if buf.can_redo() { "R" } else { "-" };
    let pos_str  = format!(
        " {} {}  {}/{}  {}:{} ",
    ...
    let pos_start = area.right().saturating_sub(pos_str.len() as u16);
    let pos_style = Style::default().fg(FG_DIM).bg(bar_bg);
...
fn render_prompt(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let h  = area.bottom().saturating_sub(1);
    let w  = area.width;
    let r  = Rect { x: area.left(), y: h, width: w, height: 1 };
    frame.render_widget(Clear, r);

    let text  = format!("{}{}", prefix, app.prompt_input);
